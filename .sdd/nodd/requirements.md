...
| 3 | `:44` | Read-only may inspect/explain/compare/recommend but must not write, edit, delegate a writer, or create artifacts | **M** | `gate-authorize` blocks `write`/`edit`/mutating bash and the `subagent` tool when intent is `read-only` |
